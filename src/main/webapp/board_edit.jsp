				...
				<form method="post"
					...
					<div class="form-row">
						<label>구분</label> <select name="board_type">
							<option value="공지" ${board.board_type == '공지' ? 'selected' : ''}>공지</option>
							<option value="건의" ${board.board_type == '건의' ? 'selected' : ''}>건의사항</option>
						</select>
					</div>
					...
					<div class="form-actions">
						<a
							href="${pageContext.request.contextPath}/board?action=detail&board_key=${board.board_key}"
							class="btn-line">취소</a>
						<button type="submit" class="btn-primary2">수정</button>
					</div>
				</form>
			</section>
		</main>
